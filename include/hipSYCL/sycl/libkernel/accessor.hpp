...
template <access_mode M, target T = target::device, bool IsRaw = false> struct mode_tag_descriptor {
  static constexpr sycl::access_mode mode = M;
  static constexpr sycl::target target = T;
  static constexpr bool is_raw = IsRaw;
};

template <class Descriptor> struct mode_tag {
...
using read_only_tag_t = mode_tag<mode_tag_descriptor<access_mode::read, target::device>>;
using read_write_tag_t = mode_tag<mode_tag_descriptor<access_mode::read_write, target::device>>;
using write_only_tag_t = mode_tag<mode_tag_descriptor<access_mode::write, target::device>>;

using read_only_host_task_tag_t =
    mode_tag<mode_tag_descriptor<access_mode::read, target::host_task>>;
using read_write_host_task_tag_t =
    mode_tag<mode_tag_descriptor<access_mode::read_write, target::host_task>>;
using write_only_host_task_tag_t =
    mode_tag<mode_tag_descriptor<access_mode::write, target::host_task>>;

using read_only_raw_tag_t = mode_tag<mode_tag_descriptor<access_mode::read, target::device, true>>;
using read_write_raw_tag_t =
    mode_tag<mode_tag_descriptor<access_mode::read_write, target::device, true>>;
using write_only_raw_tag_t =
    mode_tag<mode_tag_descriptor<access_mode::write, target::device, true>>;
...
std::shared_ptr<rt::buffer_data_region> extract_buffer_data_region(const BufferT &buff);

template <class T, int dimensions, class AllocatorT>
sycl::range<dimensions> extract_buffer_range(const buffer<T, dimensions, AllocatorT> &buff);

template <typename T, int Dimensions, typename Accessor> class accessor_iterator {
public:
  ...
  reference operator*() const { return (*acc_ptr)[id_from_linear()]; }
  ...
  friend accessor_iterator operator+(difference_type diff, const accessor_iterator &rhs) {
    auto ret = rhs;
    ret += diff;
    return ret;
  }
  ...
  bool operator==(const accessor_iterator &other) const { return linear_id == other.linear_id; }

  bool operator!=(const accessor_iterator &other) const { return !(*this == other); }
...
private:
  ...
  template <typename, int, sycl::access_mode> friend class sycl::host_accessor;
  ...
  accessor_iterator(const Accessor *acc_ptr) : acc_ptr{acc_ptr}, linear_id{0} {}
...
template <class T>
inline detail::local_memory::address allocate_local_mem(sycl::handler &, size_t num_elements);

} // namespace detail::handler

namespace detail::accessor {

template <class T, access_mode M> struct accessor_data_type {
  using value = T;
};

template <class T> struct accessor_data_type<T, access_mode::read> {
  using value = const T;
};
...
public:
  ...
  using accessor_type = sycl::accessor<dataT, dimensions, accessmode, accessTarget, isPlaceholder>;
  using reference = typename accessor_type::reference;

  using next_subscript_proxy = subscript_proxy<dataT, dimensions, accessmode, accessTarget,
                                               isPlaceholder, current_dimension + 1>;

  ACPP_UNIVERSAL_TARGET
  subscript_proxy(const accessor_type *original_accessor, sycl::id<dimensions> current_access_id)
  ...
  ACPP_UNIVERSAL_TARGET next_subscript_proxy operator[](size_t index) const {
  ...
  ACPP_UNIVERSAL_TARGET reference operator[](size_t index) const {
    return invoke_value_access(index);
  }

  template <int D = dimensions, int C = current_dimension, access_mode M = accessmode,
            std::enable_if_t<can_invoke_access(C, D) && (M == access_mode::atomic), bool> = true>
  [[deprecated("Atomic accessors are deprecated as of SYCL 2020")]] ACPP_UNIVERSAL_TARGET auto
...
template <typename dataT, int dimensions, int current_dimension = 1> class local_subscript_proxy {
  ACPP_UNIVERSAL_TARGET
  static constexpr bool can_invoke_access(int current_dim, int dim) {
    return current_dim == dim - 1;
  }
...
public:
  ...
  using next_subscript_proxy = local_subscript_proxy<dataT, dimensions, current_dimension + 1>;
...
template <class AccessorType> glue::unique_id get_unique_id(const AccessorType &acc);

struct accessor_properties {
public:
  accessor_properties() : _flags{0} {}

  accessor_properties(bool is_placeholder, bool is_no_init) : _flags{0} {
    if (is_placeholder)
  ...
  bool is_placeholder() const { return (_flags & bit_placeholder) != 0; }

  bool is_no_init() const { return (_flags & bit_no_init) != 0; }
...
template <int Dim> struct access_range {
  sycl::id<Dim> offset;
...
inline constexpr bool stores_buffer_range(accessor_variant V) { return V != accessor_variant::raw; }

inline constexpr bool stores_access_range(accessor_variant V) {
  if (V == accessor_variant::unranged || V == accessor_variant::unranged_placeholder ||
      V == accessor_variant::raw)
    return false;
  return true;
}
...
template <class TagT, bool Enable, class T> class conditional_storage {
public:
...
protected:
  ...
  static constexpr bool value_exists() { return Enable; }
...
template <class TagT, class T> class conditional_storage<TagT, true, T> {
public:
  conditional_storage() = default;
  conditional_storage(const T &v) : _val{v} {}

  conditional_storage(const conditional_storage &) = default;
  // We are copying from a type that doesn't store
  conditional_storage(const conditional_storage<TagT, false, T> &) : _val{} {}

protected:
  using value_type = T;

  static constexpr bool value_exists() { return true; }
...
using conditional_access_range_storage =
    conditional_storage<conditional_storage_access_range_tag_t, Enable, access_range<Dim>>;

template <bool Enable, int Dim>
using conditional_buffer_range_storage =
    conditional_storage<conditional_storage_buffer_range_tag_t, Enable, sycl::range<Dim>>;

template <bool Enable>
using conditional_accessor_properties_storage =
    conditional_storage<conditional_storage_accessor_properties_tag_t, Enable, accessor_properties>;

} // namespace accessor

inline access_mode get_effective_access_mode(access_mode accessmode, bool is_no_init) {
...
/// for the accessor.
template <class T> class accessor_base {
protected:
  // Will hold the actual USM pointer after scheduling
  glue::embedded_pointer<T> _ptr;
};

template <class T> inline constexpr auto default_access_tag() {
...
template <class T> inline constexpr sycl::access_mode default_access_mode() {
  return std::is_const_v<T> ? access_mode::read : access_mode::read_write;
}

template <target accessTarget, typename value_type> struct multi_ptr_for_target {
  using type = void *;
};

template <typename value_type> struct multi_ptr_for_target<target::device, value_type> {
  using type = global_ptr<value_type>;
};

template <typename value_type> struct multi_ptr_for_target<target::local, value_type> {
...
class HIPSYCL_EMPTY_BASES accessor
  ...
  static constexpr bool has_buffer_pointer =
      detail::accessor::stores_buffer_pointer(AccessorVariant);
  static constexpr bool has_access_range = detail::accessor::stores_access_range(AccessorVariant);
  static constexpr bool has_buffer_range = detail::accessor::stores_buffer_range(AccessorVariant);
  static constexpr bool has_accessor_properties =
      detail::accessor::stores_accessor_properties(AccessorVariant);

  static constexpr bool is_raw_accessor = AccessorVariant == accessor_variant::raw;
  ...
  static constexpr bool has_subscript_operators = has_size_queries || (dimensions <= 1);
  ...
  friend glue::unique_id detail::accessor::get_unique_id(const AccessorType &acc);
  ...
  static constexpr bool is_ranged_variant(accessor_variant v) {
    return v == accessor_variant::ranged || v == accessor_variant::ranged_placeholder;
  }

  static constexpr bool is_unranged_variant(accessor_variant v) {
    return v == accessor_variant::unranged || v == accessor_variant::unranged_placeholder;
  }

  static constexpr bool is_placeholder_variant(accessor_variant v) {
    return v == accessor_variant::unranged_placeholder || v == accessor_variant::ranged_placeholder;
  }

  static constexpr bool is_nonplaceholder_variant(accessor_variant v) {
    return v == accessor_variant::unranged || v == accessor_variant::ranged;
  }
  ...
  static constexpr bool is_variant_convertible(accessor_variant src, accessor_variant dest) {
    if (src == dest)
      ...
      return true;
    else if (is_unranged_variant(dest) && (is_ranged_variant(src) || is_std_accessor_variant(src)))
...
public:
  ...
  using value_type = typename detail::accessor::accessor_data_type<dataT, accessmode>::value;
  ...
  accessor() { TRACER_FUNCTION2ARG(accessor_construction, this->AdaptiveCpp_hash_code()); };
  ...
  accessor(buffer<dataT, 1, AllocatorT> &bufferRef, const property_list &prop_list = {}) {
    ...
    TRACER_FUNCTION2ARG(accessor_construction, this->AdaptiveCpp_hash_code());
  }
  ...
  template <typename AllocatorT, int D = dimensions,
            bool AllowPlaceholders = has_placeholder_constructors,
            std::enable_if_t<(D > 0 && AllowPlaceholders), int> = 0>
  accessor(buffer<dataT, dimensions, AllocatorT> &bufferRef, const property_list &prop_list = {}) {
    this->init(bufferRef, prop_list);
    TRACER_FUNCTION2ARG(accessor_construction, this->AdaptiveCpp_hash_code());
  }
  ...
  template <typename AllocatorT, int D = dimensions, bool AllowRanged = has_ranged_constructors,
            std::enable_if_t<(D > 0) && AllowRanged, int> = 0>
  accessor(buffer<dataT, dimensions, AllocatorT> &bufferRef, handler &commandGroupHandlerRef,
           range<dimensions> accessRange, const property_list &propList = {})
      : accessor{bufferRef, commandGroupHandlerRef, accessRange, id<dimensions>{}, propList} {}
  ...
  accessor(buffer<dataT, dimensions, AllocatorT> &bufferRef, handler &commandGroupHandlerRef,
    ...
    this->init(bufferRef, commandGroupHandlerRef, accessOffset, accessRange, propList);
    TRACER_FUNCTION2ARG(accessor_construction, this->AdaptiveCpp_hash_code());
  }
  ...
  accessor(const accessor &other)
      : detail::accessor_base<std::remove_const_t<dataT>>{other},
        detail::accessor::conditional_buffer_pointer_storage<has_buffer_pointer>{other},
        detail::accessor::conditional_access_range_storage<has_access_range, dimensions>{
            detail::accessor::access_range<dimensions>{other.get_offset(), other.get_range()}},
        detail::accessor::conditional_buffer_range_storage<has_buffer_range, dimensions>{other},
        detail::accessor::conditional_accessor_properties_storage<has_accessor_properties>{other} {
    TRACER_FUNCTION2ARG(accessor_construction, this->AdaptiveCpp_hash_code());
  }

  ACPP_UNIVERSAL_TARGET
  accessor &operator=(const accessor &other) {
    TRACER_FUNCTION2ARG(accessor_destruction, this->AdaptiveCpp_hash_code());
    if (*this != other) {
      detail::accessor_base<std::remove_const_t<dataT>>::operator=(other);
      detail::accessor::conditional_buffer_pointer_storage<has_buffer_pointer>::operator=(other);
      detail::accessor::conditional_access_range_storage<has_access_range, dimensions>::operator=(
          other);
      detail::accessor::conditional_buffer_range_storage<has_buffer_range, dimensions>::operator=(
          other);
      detail::accessor::conditional_accessor_properties_storage<has_accessor_properties>::operator=(
          other);
    }
  ...
  accessor &operator=(accessor &&other) {
    TRACER_FUNCTION2ARG(accessor_destruction, this->AdaptiveCpp_hash_code());
    if (*this != other) {
      detail::accessor_base<std::remove_const_t<dataT>>::operator=(std::move(other));
      detail::accessor::conditional_buffer_pointer_storage<has_buffer_pointer>::operator=(
          std::move(other));
      detail::accessor::conditional_access_range_storage<has_access_range, dimensions>::operator=(
          std::move(other));
      detail::accessor::conditional_buffer_range_storage<has_buffer_range, dimensions>::operator=(
  ...
  template <access::placeholder P, access_mode M = accessmode,
            std::enable_if_t<M == access_mode::read && is_variant_convertible(P, AccessorVariant),
                             int> = 0>
  ACPP_UNIVERSAL_TARGET accessor(const accessor<std::remove_const_t<dataT>, dimensions,
  ...
  template <
      accessor_variant OtherV,
      std::enable_if_t<is_variant_convertible(OtherV, AccessorVariant) && OtherV != AccessorVariant,
                       int> = 0>
  ACPP_UNIVERSAL_TARGET
  accessor(const accessor<dataT, dimensions, accessmode, accessTarget, OtherV> &other)
      : detail::accessor_base<std::remove_const_t<dataT>>{other},
        detail::accessor::conditional_buffer_pointer_storage<has_buffer_pointer>{other},
        detail::accessor::conditional_access_range_storage<has_access_range, dimensions>{
            detail::accessor::access_range<dimensions>{other.get_offset(), other.get_range()}},
  ...
  ~accessor() { TRACER_FUNCTION2ARG(accessor_destruction, this->AdaptiveCpp_hash_code()); }
  ...
  operator==(const accessor &lhs,
             const accessor<dataT, dimensions, accessmode, accessTarget, OtherV> &rhs) noexcept {
    if (detail::accessor::get_unique_id(lhs) != detail::accessor::get_unique_id(rhs))
      return false;

    if constexpr (AccessorVariant != accessor_variant::raw && OtherV != accessor_variant::raw &&
                  dimensions > 0) {
    ...
    if constexpr (has_accessor_properties) {
      return this
          ->detail::accessor::conditional_accessor_properties_storage<
              has_accessor_properties>::get()
          .is_placeholder();
    } else if constexpr (AccessorVariant == accessor_variant::ranged_placeholder ||
                         AccessorVariant == accessor_variant::unranged_placeholder) {
      return true;
    } else {
      return false;
  ...
  template <bool IsAllowed = has_size_queries, std::enable_if_t<IsAllowed, int> = 0>
  ACPP_UNIVERSAL_TARGET size_t get_size() const noexcept {
  ...
  ACPP_UNIVERSAL_TARGET range<dimensions> get_range() const noexcept {
    if constexpr (has_access_range) {
      return this
          ->detail::accessor::conditional_access_range_storage<has_access_range, dimensions>::ptr()
          ->range;
    } else if constexpr (has_buffer_range) {
      return this
          ->detail::accessor::conditional_buffer_range_storage<has_buffer_range, dimensions>::get();
    } else {
  ...
  /* Available only when: dimensions > 0 */
  template <int D = dimensions, std::enable_if_t<(D > 0), int> = 0>
  ACPP_UNIVERSAL_TARGET id<dimensions> get_offset() const noexcept {
    if constexpr (!has_access_range) {
  ...
  ACPP_UNIVERSAL_TARGET operator reference() const noexcept {
  ...
  template <int D = dimensions, access::mode M = accessmode,
            ...
            std::enable_if_t<(D > 1) && IsAllowed && (M != access::mode::atomic), bool> = true>
  ACPP_UNIVERSAL_TARGET reference operator[](id<dimensions> index) const noexcept {
  ...
  operator atomic<dataT, access::address_space::global_space>() const noexcept {
    return atomic<dataT, access::address_space::global_space>{global_ptr<dataT>(this->_ptr.get())};
  }

  /* Available only when: accessMode == access::mode::atomic && dimensions > 0*/
  template <int D = dimensions, access::mode M = accessmode,
            bool IsAllowed = has_subscript_operators,
            typename = std::enable_if_t<(D > 0) && IsAllowed && (M == access::mode::atomic)>>
  [[deprecated("Atomic accessors are deprecated as of SYCL "
               "2020")]] ACPP_UNIVERSAL_TARGET atomic<dataT, access::address_space::global_space>
  operator[](id<dimensions> index) const noexcept {
    return atomic<dataT, access::address_space::global_space>{
        global_ptr<dataT>(this->_ptr.get() + get_linear_id(index + get_offset()))};
  }

  template <int D = dimensions, access::mode M = accessmode,
            bool IsAllowed = has_subscript_operators,
            typename = std::enable_if_t<(D == 1) && IsAllowed && (M == access::mode::atomic)>>
  [[deprecated("Atomic accessors are deprecated as of SYCL "
               "2020")]] ACPP_UNIVERSAL_TARGET atomic<dataT, access::address_space::global_space>
  ...
  ACPP_UNIVERSAL_TARGET detail::accessor::subscript_proxy<dataT, dimensions, accessmode,
  ...
  operator[](size_t index) const noexcept {
    ...
    return detail::accessor::subscript_proxy<dataT, dimensions, accessmode, accessTarget,
                                             AccessorVariant>{this, initial_index};
  }
  ...
  ACPP_UNIVERSAL_TARGET global_ptr<dataT> get_pointer() const noexcept {
  ...
  ACPP_UNIVERSAL_TARGET constant_ptr<dataT> get_pointer() const noexcept {
    return constant_ptr<dataT>{const_cast<dataT *>(this->_ptr.get())};
  }

  /* Available only when: accessTarget == access::target::device */
  template <access::decorated IsDecorated, access::target T = accessTarget,
            typename = std::enable_if_t<T == access::target::device>>
  HIPSYCL_UNIVERSAL_TARGET accessor_ptr<IsDecorated> get_multi_ptr() const noexcept {
    return accessor_ptr<IsDecorated>{this->_ptr.get()};
  }

  iterator begin() const noexcept { return iterator::make_begin(this); }

  iterator end() const noexcept { return iterator::make_end(this); }

  const_iterator cbegin() const noexcept { return const_iterator::make_begin(this); }
...
private:
  ...
  ACPP_UNIVERSAL_TARGET
  static constexpr int get_dimensions() noexcept { return dimensions; }

  // Only valid until the embedded pointer has been initialized
  ACPP_HOST_TARGET
  glue::unique_id get_uid() const noexcept { return this->_ptr.get_uid(); }

  template <class BufferT>
  void init(BufferT &buff, id<adj_dimensions> offset, range<adj_dimensions> access_range,
            const property_list &prop_list) {
    ...
    if constexpr (has_accessor_properties) {
      ...
      this->detail::accessor::conditional_accessor_properties_storage<
          has_accessor_properties>::attempt_set(detail::accessor::accessor_properties{
  ...
  template <class BufferT> void init(BufferT &buff, const property_list &prop_list) {
  ...
  void init(BufferT &buff, handler &cgh, id<adj_dimensions> offset,
    ...
    bind_to_buffer(buff, offset, access_range);
    detail::accessor::bind_to_handler(*this, cgh, detail::extract_buffer_data_region(buff), offset,
                                      access_range, is_no_init_access);
  }

  template <class BufferT> void init(BufferT &buff, handler &cgh, const property_list &prop_list) {
    init(buff, cgh, id<adj_dimensions>{}, detail::extract_buffer_range(buff), prop_list);
  }
  ...
  range<adj_dimensions> get_buffer_shape() const noexcept {
    if constexpr (has_buffer_range) {
      return this->detail::accessor::conditional_buffer_range_storage<has_buffer_range,
                                                                      adj_dimensions>::get();
    } else {
  ...
  std::shared_ptr<rt::buffer_data_region> get_data_region() const noexcept {
    if constexpr (has_buffer_pointer) {
      return this->detail::accessor::conditional_buffer_pointer_storage<has_buffer_pointer>::get()
          .get_shared_ptr();
    }
    return nullptr;
  }
  ...
  void bind_to_buffer(BufferType &buff, sycl::id<adj_dimensions> accessOffset,
                      sycl::range<adj_dimensions> accessRange) {
    __acpp_if_target_host(
        auto buffer_region = detail::extract_buffer_data_region(buff);
        this->detail::accessor::conditional_buffer_pointer_storage<has_buffer_pointer>::attempt_set(
            detail::mobile_shared_ptr{buffer_region}););

    this->detail::accessor::conditional_access_range_storage<has_access_range, adj_dimensions>::
        attempt_set(detail::accessor::access_range<adj_dimensions>{accessOffset, accessRange});

    this->detail::accessor::conditional_buffer_range_storage<
        has_buffer_range, adj_dimensions>::attempt_set(detail::extract_buffer_range(buff));
  }
  ...
  void init_host_buffer(rt::runtime *rt, bool is_no_init) {
    ...
    auto data_mobile_ptr =
        this->detail::accessor::conditional_buffer_pointer_storage<has_buffer_pointer>::get();
    ...
    rt::dag_node_ptr node;
    {
      ...
      auto explicit_requirement = rt::make_operation<rt::buffer_memory_requirement>(
          data,
          detail::get_effective_offset<dataT, adj_dimensions>(data, rt::make_id(acc_offset),
                                                              buffer_shape, has_access_range),
          detail::get_effective_range<dataT, adj_dimensions>(data, rt::make_range(acc_range),
                                                             buffer_shape, has_access_range),
          detail::get_effective_access_mode(accessmode, is_no_init), accessTarget);

      rt::cast<rt::buffer_memory_requirement>(explicit_requirement.get())->bind(this->get_uid());

      rt::execution_hints enforce_bind_to_host;
      enforce_bind_to_host.set_hint(rt::hints::bind_to_device{detail::get_host_device()});

      node = build.builder()->add_explicit_mem_requirement(
          std::move(explicit_requirement), rt::requirements_list{rt}, enforce_bind_to_host);
...
template <class T, int Dim = 1,
          access_mode M = (std::is_const_v<T> ? access_mode::read : access_mode::read_write),
...
template <class T, int Dim = 1,
          access_mode M = (std::is_const_v<T> ? access_mode::read : access_mode::read_write),
          target Tgt = target::device>
using ranged_placeholder_accessor = accessor<T, Dim, M, Tgt, accessor_variant::ranged_placeholder>;
...
accessor(buffer<T, Dim, AllocatorT> &bufferRef, detail::mode_tag<TagDescT> tag,
         const property_list &prop_list = {})
    -> accessor<T, Dim, detail::mode_tag<TagDescT>::mode, detail::mode_tag<TagDescT>::target,
                HIPSYCL_ACCESSOR_VARIANT_SELECTOR(detail::mode_tag<TagDescT>{},
                                                  accessor_variant::true_t,
...
template <typename T, int Dim, typename AllocatorT, typename TagDescT>
accessor(buffer<T, Dim, AllocatorT> &bufferRef, range<Dim> accessRange, id<Dim> accessOffset,
         detail::mode_tag<TagDescT> tag, const property_list &propList = {})
    -> accessor<T, Dim, detail::mode_tag<TagDescT>::mode, detail::mode_tag<TagDescT>::target,
                HIPSYCL_ACCESSOR_VARIANT_SELECTOR(detail::mode_tag<TagDescT>{},
                                                  accessor_variant::true_t,
...
template <typename T, int Dim, typename AllocatorT, typename TagDescT>
accessor(buffer<T, Dim, AllocatorT> &bufferRef, handler &commandGroupHandlerRef,
         range<Dim> accessRange, detail::mode_tag<TagDescT> tag, const property_list &propList = {})
    -> accessor<T, Dim, detail::mode_tag<TagDescT>::mode, detail::mode_tag<TagDescT>::target,
                HIPSYCL_ACCESSOR_VARIANT_SELECTOR(detail::mode_tag<TagDescT>{},
                                                  accessor_variant::false_t,
...
template <typename T, int Dim, typename AllocatorT, typename TagDescT>
accessor(buffer<T, Dim, AllocatorT> &bufferRef, handler &commandGroupHandlerRef,
         range<Dim> accessRange, id<Dim> accessOffset, detail::mode_tag<TagDescT> tag,
         const property_list &propList = {})
    -> accessor<T, Dim, detail::mode_tag<TagDescT>::mode, detail::mode_tag<TagDescT>::target,
                HIPSYCL_ACCESSOR_VARIANT_SELECTOR(detail::mode_tag<TagDescT>{},
                                                  accessor_variant::false_t,
...
accessor(buffer<T, Dim, AllocatorT> &bufferRef, const property_list &prop_list = {})
    -> accessor<T, Dim, detail::default_access_mode<T>(), target::device,
...
template <typename T, int Dim, typename AllocatorT>
accessor(buffer<T, Dim, AllocatorT> &bufferRef, handler &commandGroupHandlerRef,
         range<Dim> accessRange, id<Dim> accessOffset, const property_list &propList = {})
    -> accessor<T, Dim, detail::default_access_mode<T>(), target::device,
                HIPSYCL_ACCESSOR_VARIANT_SELECTOR(detail::default_access_tag<T>(),
                                                  accessor_variant::false_t,
                                                  accessor_variant::ranged)>;
...
class host_accessor {
  ...
  template <typename DataT2, int Dim2, access_mode M2> friend class host_accessor;

  template <class TagT> void validate_host_accessor_tag(TagT tag) {
...
public:
  ...
  host_accessor() {
    TRACER_FUNCTION2ARG(host_accessor_construction, this->AdaptiveCpp_hash_code());
  };
  ...
  host_accessor(buffer<dataT, dimensions, AllocatorT> &bufferRef, detail::mode_tag<TagDescT> tag,
                const property_list &propList = {})
  ...
  template <access_mode M = accessMode, std::enable_if_t<M == access_mode::read, bool> = true>
  host_accessor(
      const host_accessor<std::remove_const_t<dataT>, dimensions, access_mode::read_write> &other)
      : _impl{other._impl} {
            TRACER_FUNCTION2ARG(host_accessor_construction, this->AdaptiveCpp_hash_code())};

  host_accessor(const host_accessor &other)
      : _impl(other._impl) {
          TRACER_FUNCTION2ARG(host_accessor_construction, this->AdaptiveCpp_hash_code())
        };

  host_accessor(host_accessor &&other) : _impl(std::move(other._impl)) {
    TRACER_FUNCTION2ARG(host_accessor_construction, this->AdaptiveCpp_hash_code());
  };

  host_accessor &operator=(const host_accessor &other) {
    TRACER_FUNCTION2ARG(host_accessor_destruction, this->AdaptiveCpp_hash_code())
    if (*this != other) {
  ...
  host_accessor &operator=(host_accessor &&other) {
    TRACER_FUNCTION2ARG(host_accessor_destruction, this->AdaptiveCpp_hash_code())
    if (*this != other) {
      this->_impl = other._impl;
    }
  ...
  ~host_accessor() { TRACER_FUNCTION2ARG(host_accessor_destruction, this->AdaptiveCpp_hash_code()) }
  ...
  std::add_pointer_t<value_type> get_pointer() const noexcept { return _impl.get_pointer(); }
  ...
  std::size_t AdaptiveCpp_hash_code() const { return _impl.AdaptiveCpp_hash_code(); }
...
template <typename T, int Dim, typename AllocatorT, typename TagDescT>
host_accessor(buffer<T, Dim, AllocatorT> &bufferRef, detail::mode_tag<TagDescT> tag,
              const property_list &prop_list = {})
...
template <typename T, int Dim, typename AllocatorT, typename TagDescT>
host_accessor(buffer<T, Dim, AllocatorT> &bufferRef, range<Dim> accessRange, id<Dim> accessOffset,
              detail::mode_tag<TagDescT> tag, const property_list &propList = {})
    -> host_accessor<T, Dim, detail::mode_tag<TagDescT>::mode>;
...
template <typename T, int Dim, typename AllocatorT>
host_accessor(buffer<T, Dim, AllocatorT> &bufferRef, const property_list &prop_list = {})
    -> host_accessor<T, Dim, detail::default_access_tag<T>().mode>;
...
template <typename dataT, int dimensions, access::mode accessmode,
          access::placeholder isPlaceholder>
class [[deprecated("use sycl::local_accessor instead")]]
accessor<dataT, dimensions, accessmode, access::target::local, isPlaceholder> {
...
public:
  ...
  std::size_t AdaptiveCpp_hash_code() const { return _addr; }
  ...
  ACPP_KERNEL_TARGET size_t get_size() const {
    return get_count() * sizeof(dataT);
  }

  [[deprecated("get_count() was removed for SYCL 2020, use size() instead")]]
  ACPP_KERNEL_TARGET size_t get_count() const {
  ...
  range<dimensions> get_range() const { return _num_elements; }

  template <int D = dimensions, access_mode M = accessmode,
            std::enable_if_t<(D == 0) && (M != access_mode::atomic), bool> = false>
  ACPP_KERNEL_TARGET operator reference() const {
    return *detail::local_memory::get_ptr<dataT>(_addr);
  }

  template <int D = dimensions, access_mode M = accessmode,
            std::enable_if_t<(D > 0) && (M != access_mode::atomic), bool> = false>
  ACPP_KERNEL_TARGET reference operator[](id<dimensions> index) const {
    return *(detail::local_memory::get_ptr<dataT>(_addr) +
  ...
  ACPP_KERNEL_TARGET reference operator[](size_t index) const {
  ...
  ACPP_KERNEL_TARGET detail::accessor::subscript_proxy<dataT, dimensions, accessmode,
                                                       access::target::local, isPlaceholder>
  operator[](size_t index) const {
    sycl::id<dimensions> initial_index;
    initial_index[0] = index;

    return detail::accessor::subscript_proxy<dataT, dimensions, accessmode, access::target::local,
...
public:
  ...
  local_accessor() {
    TRACER_FUNCTION2ARG(local_accessor_construction, this->AdaptiveCpp_hash_code())
  };
  ...
  local_accessor(const local_accessor &other)
      : _addr{other._addr}, _num_elements{other._num_elements} {
          TRACER_FUNCTION2ARG(local_accessor_construction, this->AdaptiveCpp_hash_code())
        };

  local_accessor(local_accessor &&other)
      : _addr{std::move(other._addr)}, _num_elements{std::move(other._num_elements)} {
          TRACER_FUNCTION2ARG(local_accessor_construction, this->AdaptiveCpp_hash_code())
        };

  local_accessor &operator=(const local_accessor &other) {
    TRACER_FUNCTION2ARG(local_accessor_destruction, this->AdaptiveCpp_hash_code())
    if (*this != other) {
  ...
  local_accessor &operator=(local_accessor &&other) {
    TRACER_FUNCTION2ARG(local_accessor_destruction, this->AdaptiveCpp_hash_code())
    if (*this != other) {
      this->_addr = std::move(other._addr);
      this->_num_elements = std::move(other._num_elements);
    }
    TRACER_FUNCTION2ARG(local_accessor_construction, this->AdaptiveCpp_hash_code())
  }
  ...
  /* Available only when: dimensions > 1 */
  template <int D = dimensions, std::enable_if_t<(D > 1), bool> = false>
  ACPP_KERNEL_TARGET detail::accessor::local_subscript_proxy<dataT, dimensions>
  operator[](size_t index) const {
    sycl::id<dimensions> initial_index;
    initial_index[0] = index;

    return detail::accessor::local_subscript_proxy<dataT, dimensions>{this, initial_index};
  }

  [[deprecated("use get_multi_ptr()")]]
  ACPP_KERNEL_TARGET local_ptr<dataT> get_pointer() const noexcept {
    return local_ptr<dataT>{detail::local_memory::get_ptr<dataT>(_addr)};
  }

  template <access::decorated IsDecorated>
  ACPP_KERNEL_TARGET accessor_ptr<IsDecorated> get_multi_ptr() const noexcept {
    return accessor_ptr<IsDecorated>{detail::local_memory::get_ptr<dataT>(_addr)};
  }

  template <typename T = dataT,
            std::enable_if_t<std::is_same_v<T, dataT> && !std::is_const_v<T>, bool> = false>
...
private:
  ACPP_KERNEL_TARGET
  local_accessor(address addr, range<dimensions> r) : _addr{addr}, _num_elements{r} {}
...
template <typename dataT, int dimensions, hipsycl::sycl::access_mode accessmode,
          hipsycl::sycl::target accessTarget, hipsycl::sycl::accessor_variant AccessorVariant>
struct hash<hipsycl::sycl::accessor<dataT, dimensions, accessmode, accessTarget, AccessorVariant>> {
  std::size_t operator()(const hipsycl::sycl::accessor<dataT, dimensions, accessmode, accessTarget,
...
struct hash<hipsycl::sycl::host_accessor<dataT, dimensions, A>> {
  ...
  std::size_t operator()(const hipsycl::sycl::host_accessor<dataT, dimensions, A> &acc) const {
    return acc.AdaptiveCpp_hash_code();
  }
};
...
struct hash<hipsycl::sycl::local_accessor<dataT, dimensions>> {
  ...
  std::size_t operator()(const hipsycl::sycl::local_accessor<dataT, dimensions> &acc) const {
    return acc.AdaptiveCpp_hash_code();
  }
};
